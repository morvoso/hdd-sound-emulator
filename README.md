...
[![Audio: 68+ Drives](https://img.shields.io/badge/Sound%20Libraries-68%2B%20Vintage%20HDDs-orange.svg)]()
...
- **⚡ Zero Overhead & Ultra-Lightweight (`/proc/diskstats`)**: Reads kernel block device counters directly from memory (`/proc/diskstats`) every `45ms` (takes `< 0.02ms`). Uses practically `0.00%` idle CPU without spawning messy external polling loops.
- **🎧 68+ Authentic HDD & Floppy Sound Libraries (`hdd-sounds/`)**:
  - Automatically discovers over **64 classic hardware sample folders** curated by **Deervo's DiskClick** (including the *1987 Seagate ST-225*, *1990 WD Caviar 140*, *1996 Quantum Fireball 540AT*, *2001 IBM Deskstar 75GXP*, plus *3.5" & 5.25" Floppy Drives*!).
...
- **🎲 Dynamic Randomized WAV Selection per Click**:
  - Whenever disk activity is detected, our audio engine **randomly chooses one of the `.wav` files inside the selected sound folder** (`load.wav`, `load2.wav`, `looping.wav`, `load3.wav`). This creates organic, non-repetitive mechanical chatter that feels completely alive!
...
- **🖥️ Categorized System Tray Menu & Dashboard**:
  ...
  - **Interactive Sound Dashboard**: Double-click the tray icon to open our Qt6 dashboard where you can filter profiles by era, click **`Preview Random Sound from Folder`** to audition any drive instantly, and adjust volume or sensitivity.
...
We built a simple, automated installer script that works across all major Linux distributions (Ubuntu, Debian, Fedora, Arch, Linux Mint, etc.).
...
## ▶️ How to Start, Stop & Manage

### Start the Emulator
You can launch it from your desktop application menu (`Retro 90s HDD Sound Emulator`), or via terminal:
```bash
...
./uninstall.sh
```
*This cleanly stops any running instances and removes your desktop/systemd shortcuts (`~/.local/share/applications` and `~/.config/systemd/user`). Your repository files remain untouched unless you choose to delete the directory manually (`rm -rf hdd-sound-emulator`).*
...
- The incredible library of vintage hard disk and floppy drive `.wav` sample folders inside `hdd-sounds/` (`1980s`, `1990s`, `2000s`, etc.) is curated from **DiskClick** created by **Deervo**.
...
---
*Enjoy your nostalgic 90s Linux desktop! 💾✨*
